...
let crc8cdma = function () {
    ...
    this.digest = function (data) {
        ...
        data.forEach(function (n) {
            let index = sum ^ n;
            sum = ((sum << 8) & 0xFF) ^ lookupTable[index];
        });
    };
...
let crc64ecma = function () {
    ...
    let sum = new Long(0xFFFFFFFF, 0xFFFFFFFF);

    let finalXOR = new Long(0xFFFFFFFF, 0xFFFFFFFF);
    ...
    this.digest = function (data) {
        ...
        data.forEach(function (n) {
            let index = sum.shiftRightUnsigned(56).xor(n).toNumber();
            sum = sum.shiftLeft(8).xor(lookupTable[index]);
        });
    };
    ...
    this.getLong = function () {
        return sum.xor(finalXOR);
    };
};

function reverse_u64(v) {
...
function decrypt_inner(base, data, hashed_key, output) {
    let len = data.length;
    ...
    let crc = new crc64ecma();

    crc.digest([m]);
    crc.digest(sum);

    let hashed_vec = crc.getByteArray();

    let path = [];

    for (let i = 0; i < len; ++i) {
        let index = i % 8;
        path.push((hashed_vec[index] ^ hashed_key[index]) % len);
    }

    let path_len_dec = path.length - 1;

    path.reverse().forEach(function (p, i) {
        i = path_len_dec - i;
        if (p === i) {
            return;
        }
    ...
    data.forEach(function (d, i) {
        let offset = hashed_key[i % 8] ^ base;
...
export default class ShortCrypt {
    constructor(key) {
        ...
        this._hashed_key = crc.getByteArray();

        let key_sum = Long.UZERO;

        data.forEach(function (n) {
            key_sum = key_sum.add(n);
        });

        this._key_sum_rev = reverse_u64(key_sum);
    }
    ...
    encrypt(data) {
        ...
        let crc = new crc8cdma();

        crc.digest(data);

        let hashed_value = crc.getNumber();
        ...
        let hashed_key = this._hashed_key;
    ...
    decrypt(base, body) {
        ...
        decrypt_inner(base, data, this._hashed_key, decrypted);
    ...
    encryptToURLComponent(data) {
        ...
        let cipher = this.encrypt(data);

        let base = cipher.base;

        let encrypted = cipher.body;

        base = u8_to_string_64(base);

        let base_char = String.fromCharCode(base);

        let result = base64.encode(encrypted).replace(/[+\/]/g, function (m0) {
            return m0 === '+' ? '-' : '_';
        }).replace(/=/g, '');

        let result_array = stringToUtf8ByteArray(result);

        let len = result_array.length;

        let sum = Long.fromNumber(base);

        result_array.forEach(function (n) {
            sum = sum.add(Long.fromNumber(n, true));
        });

        let base_index = this._key_sum_rev.xor(sum).mod(len + 1).toNumber();

        return result.substring(0, base_index) + base_char + result.substring(base_index, len);
    }

    decryptURLComponent(url_component) {
        let bytes = stringToUtf8ByteArray(url_component);
        ...
        let base_index = this._key_sum_rev.xor(sum).mod(len).toNumber();
        ...
        let encrypted_base64_url = url_component.slice(0, base_index) + url_component.slice(base_index + 1, len);

        // no need
        // encrypted_base64_url = encrypted_base64_url.replace(/[\-_]/g, function (m0) {
        //     return m0 == '-' ? '+' : '/';
        // });

        let encrypted = base64.decode.bytes(encrypted_base64_url);
    ...
    encryptToQRCodeAlphanumeric(data) {
        ...
        base = u8_to_string_32(base);

        let base_char = String.fromCharCode(base);

        let result = base32.encode(encrypted).replace(/=/g, '');
    ...
    decryptQRCodeAlphanumeric(qr_code_alphanumeric) {
        let bytes = stringToUtf8ByteArray(qr_code_alphanumeric);
        let len = bytes.length;
        ...
        let encrypted_base32 = qr_code_alphanumeric.slice(0, base_index) + qr_code_alphanumeric.slice(base_index + 1, len);

        let encrypted = base32.decode.asBytes(encrypted_base32);
